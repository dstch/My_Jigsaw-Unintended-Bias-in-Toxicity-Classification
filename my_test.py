...
from keras.layers import Bidirectional, GlobalMaxPooling1D, GlobalAveragePooling1D, Activation
...
def build_model(embedding_matrix, X_train, y_train, X_valid, y_valid):
    ...
    cap = Flatten()(Capsule(num_capsule=16, dim_capsule=16, routings=3, share_weights=True)(x))
    hidden = concatenate([att, GlobalMaxPooling1D()(x), GlobalAveragePooling1D()(x), cap])
