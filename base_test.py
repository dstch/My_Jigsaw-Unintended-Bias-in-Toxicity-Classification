...
@file: base_test.py
@time: 2019/4/19 18:06
@desc: fork from https://www.kaggle.com/tarunpaparaju/jigsaw-competition-eda-and-modeling
"""
import numpy as np
import pandas as pd
...
avg_word_len = train_df[TEXT_COL].apply(
    lambda x: 1.0 * len(''.join(x.split())) / (len(x) - len(''.join(x.split())) + 1))
...
sns.distplot([polarity['pos'] for polarity in polarity_0], color='orange')
...
sns.distplot([polarity['compound'] for polarity in polarity_0], color='orange')
...
spell_model = gensim.models.KeyedVectors.load_word2vec_format(
    '../input/quora-insincere-questions-classification/embeddings/wiki-news-300d-1M/wiki-news-300d-1M.vec')
...
def get_model():
    ...
    embed_inp = Embedding(len(word_index) + 1, EMBED_SIZE, weights=[embedding_matrix], input_length=MAXLEN,
                          trainable=False)(inp)
